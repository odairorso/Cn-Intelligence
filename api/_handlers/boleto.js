...
export async function handleExtractBoleto(req, res) {
  ...
  try {
    ...
    const extracted = JSON.parse(result.response.text().replace(/```json|```/gi, '').trim());
...
export async function handleSaveBoletoPattern(req, res) {
  try {
    ...
    await sql`
      ...
      VALUES (${cnpjClean || null}, ${nomeNorm}, ${fornecedor}, ${descricao}, ${empresa}, ${tipo}, ${conta_contabil_id})
