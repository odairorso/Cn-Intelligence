...
export default async function handler(req, res) {
  ...
  const publicRoutes = new Set(['health', 'folha-push', 'temp-create-account']);
  ...
  return dbStorage.run(req.authUid, async () => {
    try {
      ...
      switch (route) {
        ...
        case 'temp-create-account': { const m = await import('./temp-create-account.js'); return m.handleCreateAccount(req, res); }
