...
async function run() {
  try {
    ...
    // Print details of arrays
    console.log(`oldRevenues length: ${oldRevenues.length}, newRevenues length: ${newRevenues.length}`);

    // Let's find IDs in OLD that are not in NEW, and vice versa
    const oldIds = oldRevenues.map(tx => tx.id);
    const newIds = newRevenues.map(tx => tx.id);

    const oldNotNew = oldRevenues.filter(tx => !newIds.includes(tx.id));
    const newNotOld = newRevenues.filter(tx => !oldIds.includes(tx.id));

    console.log(`Only in OLD (by ID): ${oldNotNew.length}`);
    if (oldNotNew.length > 0) {
      console.log(oldNotNew.map(tx => `${tx.id}: ${tx.fornecedor} - ${tx.vencimento} - R$ ${tx.valor}`));
    }

    console.log(`Only in NEW (by ID): ${newNotOld.length}`);
    if (newNotOld.length > 0) {
      console.log(newNotOld.map(tx => `${tx.id}: ${tx.fornecedor} - ${tx.vencimento} - R$ ${tx.valor}`));
    }

    // Check if there are duplicate IDs in either array!
    const findDuplicates = (arr) => {
      const counts = {};
      arr.forEach(id => { counts[id] = (counts[id] || 0) + 1; });
      return Object.keys(counts).filter(id => counts[id] > 1).map(id => ({ id, count: counts[id] }));
    };

    console.log("Duplicates in OLD IDs:", findDuplicates(oldIds));
    console.log("Duplicates in NEW IDs:", findDuplicates(newIds));
