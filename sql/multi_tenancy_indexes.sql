...
-- Criar índice único composto multitenant considerando soft delete (deleted_at IS NULL)
CREATE UNIQUE INDEX IF NOT EXISTS idx_transactions_boleto_unique_tenant
  ON public.transactions (uid, regexp_replace(upper(coalesce(numero_boleto, '')), '[^A-Z0-9]', '', 'g'))
